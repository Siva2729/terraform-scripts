...
resource "aws_db_proxy" "application" {
#  count = local.enabled ? 1 : 0

  name                   = "${var.env}-cf-application-proxy"
...
resource "aws_db_proxy_default_target_group" "application" {
#  count = local.enabled ? 1 : 0

  db_proxy_name = join("", aws_db_proxy.application[*].name)

  dynamic "connection_pool_config" {
    for_each = (
      var.connection_borrow_timeout != null || var.init_query != null || var.max_connections_percent_application != null ||
    ...
    content {
      ...
      max_connections_percent      = var.max_connections_percent_application
...
resource "aws_db_proxy_target" "application" {
#  count = local.enabled ? 1 : 0

  db_instance_identifier = var.db_instance_identifier
  #db_cluster_identifier  = var.db_cluster_identifier
  db_proxy_name          = join("", aws_db_proxy.application[*].name)
  target_group_name      = join("", aws_db_proxy_default_target_group.application[*].name)
}
