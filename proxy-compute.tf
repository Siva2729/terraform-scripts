...
resource "aws_db_proxy" "compute" {
#  count = local.enabled ? 1 : 0

  name                   = "${var.env}-cf-compute-proxy" 
...
resource "aws_db_proxy_default_target_group" "compute" {
#  count = local.enabled ? 1 : 0

  db_proxy_name = join("", aws_db_proxy.compute[*].name)

  dynamic "connection_pool_config" {
    for_each = (
      var.connection_borrow_timeout != null || var.init_query != null || var.max_connections_percent_compute != null ||
    ...
    content {
      ...
      max_connections_percent      = var.max_connections_percent_compute
...
resource "aws_db_proxy_target" "compute" {
#  count = local.enabled ? 1 : 0

  db_instance_identifier = var.db_instance_identifier
  #db_cluster_identifier  = var.db_cluster_identifier
  db_proxy_name          = join("", aws_db_proxy.compute[*].name)
  target_group_name      = join("", aws_db_proxy_default_target_group.compute[*].name)
}
