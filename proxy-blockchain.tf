...
resource "aws_db_proxy" "blockchain" {
#  count = local.enabled ? 1 : 0

  name                   = "${var.env}-cf-blockchain-proxy"
...
resource "aws_db_proxy_default_target_group" "blockchain" {
#  count = local.enabled ? 1 : 0

  db_proxy_name = join("", aws_db_proxy.blockchain[*].name)

  dynamic "connection_pool_config" {
    for_each = (
      var.connection_borrow_timeout != null || var.init_query != null || var.max_connections_percent_blockchain != null ||
    ...
    content {
      ...
      max_connections_percent      = var.max_connections_percent_blockchain
...
#  count = local.enabled ? 1 : 0
  ...
  db_proxy_name          = join("", aws_db_proxy.blockchain[*].name)
  target_group_name      = join("", aws_db_proxy_default_target_group.blockchain[*].name)
}
