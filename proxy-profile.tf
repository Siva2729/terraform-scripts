...
resource "aws_db_proxy" "profile" {
#  count = local.enabled ? 1 : 0

  name                   = "${var.env}-cf-profile-proxy" 
...
resource "aws_db_proxy_default_target_group" "profile" {
#  count = local.enabled ? 1 : 0

  db_proxy_name = join("", aws_db_proxy.profile[*].name)

  dynamic "connection_pool_config" {
    for_each = (
      var.connection_borrow_timeout != null || var.init_query != null || var.max_connections_percent_profile != null ||
    ...
    content {
      ...
      max_connections_percent      = var.max_connections_percent_profile
...
resource "aws_db_proxy_target" "profile" {
#  count = local.enabled ? 1 : 0

  db_instance_identifier = var.db_instance_identifier
  #db_cluster_identifier  = var.db_cluster_identifier
  db_proxy_name          = join("", aws_db_proxy.profile[*].name)
  target_group_name      = join("", aws_db_proxy_default_target_group.profile[*].name)
}
