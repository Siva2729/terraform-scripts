resource "aws_security_group" "application-lambda-sg" {
  name        = "${var.env}-${var.application-lambda-name}-sg"
...
resource "aws_lambda_permission" "with_lb" {
  ...
  function_name = aws_lambda_function.application-lambda.function_name
  principal     = "elasticloadbalancing.amazonaws.com"
  source_arn    = aws_lb_target_group.application-tg.arn
}

resource "aws_lambda_function" "application-lambda" {
  function_name = "${var.env}-${var.application-lambda-name}"
  ...
  s3_bucket     = var.application-lambda-name-s3
  s3_key        = var.application-lambda-name-s3-path

  vpc_config {
    subnet_ids         = [aws_subnet.pubsub1.id, aws_subnet.pubsub2.id, aws_subnet.pubsub3.id]
    security_group_ids = [aws_security_group.application-lambda-sg.id]
  }
#  environment {
#    variables = {
...
resource "aws_lb_target_group" "application-tg" {
  name        = "${var.env}-${var.application-lambda-name}-tg"
  target_type = "lambda"
  health_check {
    healthy_threshold   = "3"
    unhealthy_threshold = "3"
    timeout             = "5"
    path                = "/api/application/ping"
  }
}
#-------------------------------------------------------------------------------------------------#
...
resource "aws_lb_target_group_attachment" "application" {
  ...
  target_id        = aws_lambda_function.application-lambda.arn
}
