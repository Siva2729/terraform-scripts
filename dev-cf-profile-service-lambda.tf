resource "aws_security_group" "profile-lambda-sg" {

  name        = "${var.env}-${var.profile-lambda-name}-sg"
...
resource "aws_lambda_permission" "with_profile_lb" {
  statement_id  = "AllowExecutionFromlb"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.profile-lambda.function_name
  principal     = "elasticloadbalancing.amazonaws.com"
  source_arn    = aws_lb_target_group.profile-tg.arn
}

resource "aws_lambda_function" "profile-lambda" {
  function_name = "${var.env}-${var.profile-lambda-name}"
  ...
  s3_bucket     = var.cf-profile-lambda-name-s3
  s3_key        = var.cf-profile-lambda-name-s3-path

  vpc_config {
    subnet_ids         = [aws_subnet.pubsub1.id, aws_subnet.pubsub2.id, aws_subnet.pubsub3.id]
    security_group_ids = [aws_security_group.profile-lambda-sg.id]
  }
...
resource "aws_lb_target_group" "profile-tg" {
  name        = "${var.env}-${var.profile-lambda-name}-tg"
  target_type = "lambda"
  health_check {
        healthy_threshold   = "3"
    unhealthy_threshold = "3"
    timeout             = "5"
    path                = "/api/profile/ping"
  }
}
resource "aws_lb_target_group_attachment" "profile" {
  target_group_arn = aws_lb_target_group.profile-tg.arn
  target_id        = aws_lambda_function.profile-lambda.arn
}
